...
def main():
    ...
    wl, flux, ivar = bases.readdata('../../data/J1120_stack1d.fits', wlmin=33500, wlmax=40060)
    ...
    samples = pickle.load(open('../saves/J1120_mcmc_samples_step2_with_Hgamma_twocomp_v2.pickle', 'rb'))
    ...
    pos = bases.GetInitialPositionsBalls(nwalkers, best_guess, ndim, percent=0.02)
    ...
    with Pool() as pool:
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, bases.lnprob_jwst, args=(wl, flux, ivar, coef_min, coef_max, z, iron, bases.flux_model_full_with_Hgamma_twocomp),\
                                        pool=pool, moves=[(emcee.moves.DEMove(), 0.8), (emcee.moves.DESnookerMove(), 0.2)])
        sampler.run_mcmc(pos, nsteps, progress=True)

    f = open('../saves/J1120_mcmc_samples_step2_final.pickle', 'wb')
